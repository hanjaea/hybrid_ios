...
class CroppableImageView: UIView, CornerpointClientProtocol
{
    ...
    override func layoutSubviews()
    {
        ...
        if let requiredImageSize = imageSize
        {
            ...
            var size = displaySize.width
            
            var xwidth:CGFloat = 0.0
            var yheight:CGFloat = 0.0
            
            if(displaySize.width > displaySize.height){
                size = displaySize.height
                
                xwidth = (displaySize.width - (size))/2
                
            }else{
                yheight = (displaySize.height - (size))/2
            }
            
            
            var startPoint = CGPoint(x: 0 + (size/10) + xwidth, y: 0 + (size/10) + yheight)
            
            var newPoint = CGPoint(x:size - (size/10) + xwidth, y: size - (size/10) + yheight)
    ...
    func rectFromStartAndEnd(_ startPoint:CGPoint, endPoint: CGPoint) -> CGRect
    {
        ...
        if(right > (imageRect?.width)!){
            right = (imageRect?.width)!
        }
        if(bottom > (imageRect?.height)!){
            bottom = (imageRect?.height)!
        }
        ...
        let result = CGRect(x: left, y: top, width: max(width, height), height: max(width, height))
        return result
    }
    ...
    override func draw(_ rect: CGRect)
    {
        ...
        { 
            ...
            path.stroke()
            path.lineWidth = 2.0
            UIColor(red: 1.0, green: 1.0, blue: 0, alpha: 0.8).set()
            path.stroke()
        }
    }
    ...
    @objc func handleDragInView(_ thePanner: UIPanGestureRecognizer)
    {
        ...
        var newPoint = thePanner.location(in: self)
